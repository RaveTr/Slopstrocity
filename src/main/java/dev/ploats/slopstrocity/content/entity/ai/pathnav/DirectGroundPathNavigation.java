package dev.ploats.slopstrocity.content.entity.ai.pathnav;
...
public class DirectGroundPathNavigation extends GroundPathNavigation {
    ...
    protected void followThePath() {
        ...
        for (int i = curPath.getNextNodeIndex(); i < curPath.getNodeCount(); i++) {
            if (curPath.getNode(i).y != Math.floor(pathEntityTempPos.y) && hasValidPathType(curPath.getNode(i).type)) {
                pathLength = i;
                break;
            }
        }
    ...
    protected static float switchAxis(Vec3 pos, int axisIndex) {
        switch (axisIndex) {
            case 0: return (float) pos.x;
            case 1: return (float) pos.y;
            case 2: return (float) pos.z;
            default: return 0.0F;
        }
    }
    ...
    private boolean tryTruncateNodes(Path pathToTrim, int pathLength, Vec3 pathEntityPos, Vec3 center, Vec3 maxArea) {
        for (int length = pathLength; --length > pathToTrim.getNextNodeIndex();) {
            ...
            if (sweepThrough(dist, center, maxArea)) {
    ...
    protected boolean sweepThrough(Vec3 pathVec, Vec3 center, Vec3 max) {
        ...
        final PathfindingContext context = new PathfindingContext(level, mob);
        ...
        do {
            ...
            for (int x = x0; x != x1; x += stepx) {
                for (int z = z0; z != z1; z += stepz) {
                    ...
                    if (in == PathType.DAMAGE_FIRE || in == PathType.DANGER_FIRE || in == PathType.DAMAGE_OTHER || in == PathType.WATER) return false;
                }
            }
        } while (l < ml);
    ...
    protected boolean hasValidPathType(PathType type) {
        if (type == PathType.WATER) return false;
        else if (type == PathType.LAVA) return false;
        else return type != PathType.OPEN;
    }

    @Override
    public boolean canCutCorner(PathType pPathType) {
        return pPathType != PathType.LAVA && super.canCutCorner(pPathType);
    }
