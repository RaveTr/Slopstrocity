...
public class SlopstrocityLeapChequeAttackGoal extends Goal {
    ...
    private void land() {
        ...
        owner.hurtTargets(owner, owner.getTarget(), EntityUtil.getAllEntitiesAround(owner, 8.0D, 8.0D, 8.0D, 7.5D).stream().filter(entity -> !owner.isAlliedTo(entity) && !entity.isAlliedTo(owner) && owner.getClass() != entity.getClass() && EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(entity)).toList());
    }
