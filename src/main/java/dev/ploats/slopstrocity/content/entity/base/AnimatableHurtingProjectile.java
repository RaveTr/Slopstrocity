...
public abstract class AnimatableHurtingProjectile extends AbstractHurtingProjectile implements WrappedAnimatable {
    ...
    public void handleInitialMovement(Entity shooter, double xOffset, double yOffset, double zOffset) {
        handleInitialMovement(shooter, xOffset, yOffset, zOffset, true);
    }

    public void handleInitialMovement(Entity shooter, double xOffset, double yOffset, double zOffset, boolean affectPower) { // Needed to avoid unnecessary constructor-imposed updates in AbstractHurtingProjectile
        reapplyPosition();

        if (affectPower) {
            double totalOffsetDelta = Math.sqrt(xOffset * xOffset + yOffset * yOffset + zOffset * zOffset);

            if (totalOffsetDelta != 0.0D) {
                this.accelerationPower = totalOffsetDelta * 0.1D;
            }
        }

        moveTo(shooter.getX(), shooter.getEyeY(), shooter.getZ(), shooter.getYRot(), shooter.getXRot());
        setOwner(shooter);
