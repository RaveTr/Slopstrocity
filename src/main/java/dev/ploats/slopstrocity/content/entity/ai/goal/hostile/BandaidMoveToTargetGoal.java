...
public class BandaidMoveToTargetGoal extends Goal {
    ...
    public boolean canContinueToUse() {
        return isExecutable(owner.getTarget()) && owner.isWithinRestriction(owner.getTarget().blockPosition()) && failedIterations < maxFailedIterations && owner.distanceToSqr(owner.getTarget()) > (satisDist != 0.0D ? satisDist * satisDist : EntityUtil.getMeleeAttackReachSqr(owner, owner.getTarget()));
    }
    ...
    public void tick() {
        ...
        if ((pathCheckRate <= 0 || owner.getNavigation().isDone() || owner.getNavigation().isStuck()) && owner.getSensing().hasLineOfSight(target) && owner.distanceToSqr(target) > (satisDist != 0.0D ? satisDist * satisDist : EntityUtil.getMeleeAttackReachSqr(owner, owner.getTarget()))) {
    ...
    protected boolean isExecutable(LivingEntity target) {
        ...
        if (target.isAlive() && !target.isSpectator()) {
            ...
            if (owner instanceof AnimatableMonster emOwner) return emOwner.getSensing().hasLineOfSight(target) && emOwner.distanceToSqr(target) > (satisDist != 0.0D ? satisDist * satisDist : EntityUtil.getMeleeAttackReachSqr(emOwner, target)) && !emOwner.isAttackingStatically();
